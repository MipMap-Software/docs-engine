...
async function main() {
  ...
  const tempRoot = await mkdtemp(path.join(tmpdir(), "docdesktop-pandoc-"));
  try {
    ...
    const outFile = path.join(outDir, `mipmap-desktop-docs-${selectedLang}.pdf`);
    ...
    const args = [
      ...
      pdfEngine,
      "-V",
      "title=MipMap Desktop User Manual",
    ];
